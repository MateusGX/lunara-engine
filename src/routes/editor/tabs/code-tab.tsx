...
function mkInfo(desc: string, example: string): Completion["info"] {
  return () => {
    ...
    wrap.style.cssText =
      "padding:6px 8px;max-width:280px;font-size:12px;line-height:1.5";
    ...
    pre.style.cssText =
      "margin:0;padding:5px 7px;background:#1a1a2e;border:1px solid rgba(255,255,255,0.07);color:#e5c07b;font-family:inherit;font-size:11px;white-space:pre-wrap;word-break:break-all";
...
/** Extract local variable and function names declared in the document */
function parseLocalSymbols(doc: string): Completion[] {
  const seen = new Set<string>();
  const options: Completion[] = [];

  const add = (name: string, type: "variable" | "function") => {
    if (!seen.has(name)) {
      seen.add(name);
  ...
  // local a, b, c = ...
  for (const m of doc.matchAll(/\blocal\s+([\w,\s]+?)(?:\s*=|\s*$)/gm)) {
    for (const name of m[1].split(",")) {
      const trimmed = name.trim();
      if (/^\w+$/.test(trimmed)) add(trimmed, "variable");
    }
  }

  // function name(...) and local function name(...)
  for (const m of doc.matchAll(/(?:local\s+)?function\s+(\w+)\s*\(/g)) {
    add(m[1], "function");
  }
  ...
  for (const m of doc.matchAll(/^(\w+)\s*=(?!=)/gm)) {
    const name = m[1];
    if (!LUA_KEYWORDS.has(name)) add(name, "variable");
  }
...
function buildCompletions(scripts: ScriptData[]) {
  return function (context: CompletionContext) {
    ...
    return {
      from: word.from,
      options: [...LUA_API_COMPLETIONS, ...moduleOptions, ...localSymbols],
    };
  };
}
